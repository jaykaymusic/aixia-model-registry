...
## 2026-07-20 — Manual out-of-band edit: +kimi family (Moonshot AI), +agentic_assistant routing default

> ⚠️ **Not a crawler run.** Manual addition at user request to back the Hermes/AIXIA autonomous assistant. Only WebSearch was available for verification (Perplexity/Firecrawl remain network-blocked). The next scheduled crawler run (2026-07-27) should independently re-verify this entry.

- **New family added:** `kimi` · Moonshot AI · `kimi-k3` ($3/$15 per million, 1,048,576-token context, released July 16, 2026). 2.8T-param open-weight MoE (16 of 896 experts active per request), multimodal reasoning. Cache-hit input $0.30/M; built-in web search +$0.015/call. Open weights slated ~July 27 under Modified MIT license. Reported to edge out Claude Fable 5 in Frontend Code Arena (Tom's Hardware) — positioned as a frontier agentic/coding model. Promo: up to 30% API-credit voucher rebate on top-ups ≥$1,000 through Aug 12, 2026.
- **New routing default:** `routing_defaults.agentic_assistant` → `kimi`. This is the default the Hermes/AIXIA assistant should resolve. Existing category defaults (user_facing_chat, swarm_debates_and_qa, etc.) were **left unchanged** — no existing Claude/DeepSeek/Gemini traffic was redirected.
- 🚨 **Jurisdiction:** Moonshot AI is **China-hosted** — treat like `deepseek` (disclose in ToS; unsuitable for data-residency-sensitive or Privacy Mode workloads). `kimi` is deliberately **EXCLUDED from `privacy_mode_routing`** (Privacy Mode remains Claude-only). Any project enabling Privacy Mode will not resolve to Kimi even for the agentic_assistant category.
- **No changes for:** all existing families and their versions/pricing.

**Summary:** Adds Kimi K3 (Moonshot AI) as the `kimi` family and wires it in as the new `agentic_assistant` routing default, per user request, to back the Hermes/AIXIA Mac assistant. This is a manual, non-crawler edit verified only via WebSearch — flagged for re-verification at the July 27 crawl. The load-bearing caveat is jurisdictional: Kimi is China-hosted, so it is kept out of Privacy Mode routing and should be disclosed in any consuming project's ToS. Downstream action required: the Hermes app config must point its model resolution at the `kimi` family (or the `agentic_assistant` routing category) — the registry entry alone does not change which model Hermes calls.
